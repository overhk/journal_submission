...
########## normalize the data as the user always work in the original space
    ...
    def MLA_ParallelEval_(self, NS, NS1 = None, NI = None, Tgiven = None, T_sampleflag = None, function_evaluations = None, source_function_evaluations = None, models_transfer = None, numInParallel=0, lieType=0, **kwargs):
        print("Parallel eval being called")
        # for i in range(5):
        #     print("In MLA Parallel Eval")
        #     print("Num in parallel: ", numInParallel)
        stats = {
            "time_total": 0,
            "time_sample_init": 0,
            "time_fun": 0,
        ...
        num_evaluation_instances = numInParallel


        print("Num evals: ", num_evaluation_instances)
        while NSmin<NS:# YL: each iteration adds 1 (if single objective) or at most kwargs["search_more_samples"] (if multi-objective) sample until total #sample reaches NS
            ...
            print("NI: ", NI)
            print("NSMIN: ", NSmin)
            control = num_evaluation_instances
            if NI > 1:
                control += NSmin

            for evaluation_instance in range(control):
                ...
                for o in range(self.problem.DO):
                    ...
                    if evaluation_instance == 0:
                        ...
                        tmpdata = copy.deepcopy(data_replica)
                        tmpdata.O = [copy.deepcopy(data_replica.O[i][:,o].reshape((-1,1))) for i in range(len(data_replica.I))]
                        print("ADRIAN: copying from memory 2")
                ...
                print("ADRIAN: THIS IS BAD")
                #for i in range(NI):
                 #   for o in range(self.problem.DO):
                i=0
                o=0
                if lie == 0:
                    print(data_replica.O[i])
                    (mu, var) = modelers[o].predict(np.array(newdata_for_replica.P[i][0]), i)
                    newdata_for_replica.O.append(mu)
                    suggest=mu
                    print("mu appended ",mu)

                else:
                    if lie == 1:
                        #using minimum (emphasizing exploit)
                        print(data_replica.O[0])
                        statMin =min(min(data_replica.O[0]))
                        suggest = [[statMin]]
                        newdata_for_replica.O.append(suggest)
                        print("minimum appended",statMin)
                    elif lie == 2:
                        #using mean (emphasizing explore)
                        print(data_replica.O[0])
                        statMean = mean = np.mean(data_replica.O[0]) 
                        suggest =[[statMean]]
                        newdata_for_replica.O.append(suggest)
                        print("mean appended",statMean)
                        ##Need to figure out how to unpack elements for actual list sorting##
                    elif lie >= 3:
                        # a sorted list will be used in cases going forward
                        oldList = data_replica.O[0]
                        newList = [item for sublist in oldList for item in sublist]
                        newList.sort()
                        print(newList)
                        if lie == 3:
                            #median - resistant measure as alternative to mean
                            med = np.median(newList)
                            suggest = [[med]]
                            newdata_for_replica.O.append(suggest)
                            print("median appended",med)
                        elif lie == 4:
                            #lower quartile (generalize between min and median)
                            lq = np.percentile(newList, 25)
                            suggest = [[lq]]
                            newdata_for_replica.O.append(suggest)
                            print("lower quartile appended", lq)
                        elif lie >= 5:
                            #upper quartile and lower quartile
                            lq = np.percentile(newList, 25)
                            uq = np.percentile(newList, 75)
                            if uq < 0: uq = 0
                            num_data = len(data_replica.O[0])
                            #Contextual Variance:
                            #(mu, var) = modelers[o].predict(np.array(newdata_for_replica.P[i][0]), i)
                            #num_data=91*(1-var/mu) 
                            if lie == 5:
                                #min plus iqr gives another value that minimizes effect of possible large outliers (rather than using mean)
                                iqr = uq - lq
                                suggest = [[newList[0] + iqr]]
                                newdata_for_replica.O.append(suggest)
                                print("min+iqr appended",suggest)
                            elif lie == 6:
                                #avg of uq and iq minimizes impact of outliers (for greater generalization)
                                suggest = [[(uq + lq)/2]]
                                newdata_for_replica.O.append(suggest)
                                print("avg of uq and lq",suggest)
                            elif lie == 7:
                                if (num_data < 30):
                                    suggest = [[lq]]
                                    newdata_for_replica.O.append(suggest)
                                ...
                                if (num_data < 30):
                                    statMean = np.mean(data_replica.O[0])
                                    suggest = [[statMean]]
                                    newdata_for_replica.O.append(suggest)
                                else:
                                    statMin =min(min(data_replica.O[0]))
                                ...
                                if num_data < 50:
                                    ...
                                    (mu, var) = modelers[o].predict(np.array(newdata_for_replica.P[i][0]), i)
                                    newdata_for_replica.O.append(mu)
                                    # print("Data predicton is based on: ", np.array(newdata_for_replica.P[i][0]))
      
                            elif lie == 19:
                                statMax =max(max(data_replica.O[0]))
                                suggest = [[statMax]]
                                print("max appended",statMax,suggest)
                                newdata_for_replica.O.append(suggest)
                ...
                csv_file2 = open('predictions.csv','a',newline='')
                csv_writer2=csv.writer(csv_file2)
                csv_writer2.writerow(suggest)
                csv_file2.close()
            ...
            if NI == 1:
                ...
                # print([data_replica.problem.PS.inverse_transform(x) for x in data_replica.P]
                oldP = data_replica.P[tid]

                Ps = [self.problem.PS.inverse_transform(x) for x in data_replica.P]
                Os = data_replica.O[tid].tolist()


                for P,old, O in zip(Ps[0],oldP, Os):
                    print("Param: ", P ,end = '\t\t' )
                    print("Old reprsentation: ", old, end= '\t\t')
                    print("Predicted Result: ", O)


                # Assuming you have opened the CSV file in write mode
                csv_file = open('test.csv', 'w', newline='')
                csv_writer = csv.writer(csv_file)
                i = 1
                for P,O in zip(Ps[0],Os):
                    #P_last=P
                    #O_last=O

                    csv_writer.writerow([P, O[0]])
    ...
    def MLA(self, NS, NS1 = None, NI = None, Tgiven = None, numInParallel=0,lieType=0):
        ...
        if self.options["BO_objective_evaluation_parallelism"] == False:
            ...
            return self.MLA_ParallelEval_(NS, NS1, NI, Tgiven, T_sampleflag=[True]*NI, function_evaluations=None, source_function_evaluations=None, models_transfer=None, numInParallel=numInParallel,lieType=lieType)
        else:
            return self.MLA_(NS, NS1, NI, Tgiven, T_sampleflag=[True]*NI, function_evaluations=None, source_function_evaluations=None, models_transfer=None)
