...
class SurrogateProblem(object):
                    ...
                    if self.models_transfer == None:
                        if self.data.O == None:
                            ...
                            if self.options['search_af'] == 'EI':
                                #Dynamic 1: alpha=(400-len(self.data.O[0]))/400
                                #Dynamic 2: alpha=0.5*(400-len(self.data.O[0]))/400
                                ymin = self.data.O[self.tid][:,o].min()
                                (mu, var) = self.models[o].predict(x, tid=self.tid)
                                mu = mu[0][0]
                                
                                #Contextual variance: alpha=np.mean(np.mean(var))/mu
    ...
    def fitness(self, x):   # x is the normalized space
        ...
        if (CND):
            if(self.problem.models is not None):
                xNorm = np.hstack((xNorm,np.array(modeldata).reshape(self.options['search_more_samples'],1)))
